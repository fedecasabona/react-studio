...
interface MagicPlacesVideoWindowProps {
  onClose: () => void
  onMinimize: () => void
  onMaximize: () => void
  isMinimized: boolean
...
const MagicPlacesVideoWindow: React.FC<MagicPlacesVideoWindowProps> = ({
  onClose,
  onMinimize,
  onMaximize,
  isMinimized,
      ...
      >
        <div className="terminal-header flex items-center justify-between px-3 py-1 h-full">
          <div className="flex items-center space-x-2">
            <span className="text-white font-roboto font-normal uppercase tracking-wider" style={{ fontSize: '0.65rem' }}>magic places</span>
          </div>
          <button
            onClick={(e) => {
              e.stopPropagation()
      ...
      <div className="flex items-center justify-center bg-black p-4" style={{ height: 'calc(100% - 40px)' }}>
        <div className="w-full h-full flex items-center justify-center">
          <iframe 
            width="100%" 
            height="100%" 
            src="https://www.youtube.com/embed/CsS7Hv2Cw2M?si=Nc4pzCzFZHlEyyJu" 
...
export default MagicPlacesVideoWindow
